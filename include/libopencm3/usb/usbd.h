...
enum usbd_control_result {
	USBD_REQ_STALL, /**< Stall the Request */
	USBD_REQ_HANDLED, /**< Request has been handled */
	USBD_REQ_NEXT, /**< Request to be served to next handler */
};
...
/*
 * Naming convention for driver:
 *  - the name should include name of the part for which the first driver was written.
 *  - naming like "usb_driver_otgfs" (or macro) should be avoided
 *      because it do not give any information about the part.
 *  - Easier to manage when different vendors.
 *  - Part name should include the familty and sub-family.
 *     ex: family: stm32f0, sub-family: stm32f072
 *
 * commonized driver (ex: dwc_otg) should have a name
 *   that describe about the whole family of peripherial (it is intended for).
 *
 * filename and driver name should be same.
 * usb driver files prefix with "usbd_driver_"
 *  example: usb_driver_stm32f072.c or usd_driver_dwc_otg.c
 */
extern const usbd_driver usb_driver_stm32f103;
extern const usbd_driver usb_driver_stm32f107;
extern const usbd_driver usb_driver_stm32f207;
extern const usbd_driver usb_driver_stm32f072;
extern const usbd_driver usb_driver_lm4f;
