				...
				usbd_generic_callback callback)
{
	...
	if (usbd_dev->driver->enable_sof) {
		usbd_dev->driver->enable_sof(usbd_dev, callback != NULL);
	}
}
...
void usbd_disconnect(usbd_device *usbd_dev, bool disconnect)
{
	if (usbd_dev->driver->disconnect) {
		usbd_dev->driver->disconnect(usbd_dev, disconnect);
	}
}
...
{
	if(usbd_dev->driver->set_ep_type) {
		usbd_dev->driver->set_ep_type(usbd_dev, addr, type);
	}
}

void usbd_set_ep_size(usbd_device *usbd_dev, uint8_t addr, uint16_t max_size)
{
	if (usbd_dev->driver->set_ep_size) {
		usbd_dev->driver->set_ep_size(usbd_dev, addr, max_size);
	}
}
...
{
	usbd_dev->driver->set_ep_stall(usbd_dev, addr, stall);
}

bool usbd_get_ep_stall(usbd_device *usbd_dev, uint8_t addr)
{
	return usbd_dev->driver->get_ep_stall(usbd_dev, addr);
}

void usbd_set_ep_nak(usbd_device *usbd_dev, uint8_t addr, bool nak)
{
	usbd_dev->driver->set_ep_nak(usbd_dev, addr, nak);
}

void usbd_set_ep_dtog(usbd_device *usbd_dev, uint8_t addr, bool dtog)
{
	usbd_dev->driver->set_ep_dtog(usbd_dev, addr, dtog);
}

bool usbd_get_ep_dtog(usbd_device *usbd_dev, uint8_t addr)
{
	return usbd_dev->driver->get_ep_dtog(usbd_dev, addr);
}

void usbd_ep_flush(usbd_device *usbd_dev, uint8_t addr)
{
	if (usbd_dev->driver->ep_flush) {
		usbd_dev->driver->ep_flush(usbd_dev, addr);
	}
}
...
{
	if (usbd_dev->driver->frame_number) {
		return usbd_dev->driver->frame_number(usbd_dev);
	}
